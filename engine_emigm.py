...
def evaluate(model_without_ddp, vae, ema_params, args, epoch, batch_size=16, log_writer=None, cfg=1.0, temperature=1.0,
             use_ema=True):
    ...
    for i in range(num_steps):
        ...
        with torch.no_grad():
            with torch.cuda.amp.autocast():
                ...
                if args.use_dc_ae:
                    sampled_images = vae.decode(sampled_tokens / 0.3189)
                else: # use VAE
    ...
    if log_writer is not None:
        if args.img_size == 256:
            input2 = None
            fid_statistics_file = 'fid-stats/fid_stats_imagenet256_guided_diffusion.npz'
        elif args.img_size == 512:
            input2 = None 
            fid_statistics_file = 'fid-stats/fid_stats_imagenet512_guided_diffusion.npz'
        else:
        ...
        if not temperature == 1.0:
            postfix = postfix + "_temp{}".format(temperature)
...
def cache_latents_ae(ae,
    ...
    for data_iter_step, (samples, _, paths) in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
        ...
        with torch.no_grad():
            posterior = ae.encode(samples)
            moments = posterior * 0.3189 # 0.3156 is f32c32 std 0.3189 for 512 generation
            posterior_flip = ae.encode(samples.flip(dims=[3]))
            moments_flip = posterior_flip * 0.3189 # 0.3156 is f32c32 std 0.3189 for 512 generation
