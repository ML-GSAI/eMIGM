...
def model_wrapper(
    ...
    def noise_pred_fn(x, t_continuous, cond=None, use_guidance_model=False):
        t_input = get_model_input_time(t_continuous)
        if not use_guidance_model:
            if cond is None:
                output = model(x, t_input, **model_kwargs)
            else:
                output = model(x, t_input, cond, **model_kwargs)
        else:  
            assert guidance_model is not None
            if cond is None:
                output = guidance_model(x, t_input, **model_kwargs)
            else:
                output = guidance_model(x, t_input, cond, **model_kwargs)
        if model_type == "noise":
            return output
    ...
    def model_fn(x, t_continuous):
        ...
        elif guidance_type ==  "classifier-free-auto":
            assert guidance_model is not None
            if guidance_scale == 1. or unconditional_condition is None:
                return noise_pred_fn(x, t_continuous, cond=condition)
            noise_guided = noise_pred_fn(x, t_continuous, cond=unconditional_condition, use_guidance_model=True)
            noise = noise_pred_fn(x, t_continuous, cond=condition)
            return noise_guided + guidance_scale * (noise - noise_guided)

    assert model_type in ["noise", "x_start", "v", "score"]
    assert guidance_type in ["uncond", "classifier", "classifier-free", "classifier-free-auto"]
    return model_fn
